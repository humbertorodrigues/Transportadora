...
$(document).ready(function(){
    ...
    $("#myForm").submit(function(){
        $("#loader").html('&nbsp;<span class="label label-info">carregando...</span>');
        calcRoute();
        return false;
    });
...
function initialize() {
  	var mapOptions = {
      	center: new google.maps.LatLng(-8.556219, -52.916830),
      	zoom: 4,
...
function calcRoute() {
    ...
    directionsService.route(request, function(response, status) {
        ...
        if (status == google.maps.DirectionsStatus.OK) {
            ...
            $("#locations").html('<h4>Direções para :</h4>' + locations_list);
            ...
            total_summary += '  <strong>Distância total : </strong>' + showDistance(distance) + " ( tempo de viagem: " + Math.round(time/60) + " minutos)";
            total_summary += '</div>';
...
function showDistance(distance) {
    return Math.round(distance/100) / 10 + " km (" + Math.round((distance*0.621371192)/100) / 10 + " milhas)";
}
...
function add_waypoint()
{
    ...
    waypoint_container+= '  <label class="control-label" for="inputEmail">Início a partir :</label>';
    waypoint_container+= '  <div class="controls">';
    waypoint_container+= '      <input type="text" id="toWaypoints" name="toWaypoints[]" class="form-control " value="">';
    waypoint_container+= '      &nbsp;<a href="#" onclick="return add_waypoint();">Adicionar outro ponto</a>';
    waypoint_container+= '      &nbsp;<a href="#" onclick="return remove_waypoint(this);">Remover</a>';
